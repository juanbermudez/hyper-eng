# Hyper-Engineering Plugin Marketplace

This repository is a Claude Code plugin marketplace that distributes the `hyper-engineering` plugin to developers building with AI-powered tools.
...
└── plugins/
    └── hyper-engineering/        # The actual plugin
        ├── .claude-plugin/
...
## Philosophy: Hyper-Engineering

**Specs matter more than code. Code is disposable; specifications are the source of truth.**

When working on this repository, follow the hyper-engineering process:
...
### Updating the Hyper-Engineering Plugin
...
```bash
# Count agents
ls plugins/hyper-engineering/agents/*.md | wc -l

# Count commands
ls plugins/hyper-engineering/commands/*.md | wc -l

# Count skills
ls -d plugins/hyper-engineering/skills/*/ 2>/dev/null | wc -l
```
...
- [ ] `plugins/hyper-engineering/.claude-plugin/plugin.json` → `description` field
- [ ] `.claude-plugin/marketplace.json` → plugin `description` field
- [ ] `plugins/hyper-engineering/README.md` → intro paragraph
...
- [ ] `plugins/hyper-engineering/.claude-plugin/plugin.json` → `version`
- [ ] `.claude-plugin/marketplace.json` → plugin `version`

#### 4. Update documentation

- [ ] `plugins/hyper-engineering/README.md` → list all components
- [ ] `plugins/hyper-engineering/CHANGELOG.md` → document changes
...
cat plugins/hyper-engineering/.claude-plugin/plugin.json | jq .
```
...
grep -o "Includes [0-9]* specialized agents" plugins/hyper-engineering/.claude-plugin/plugin.json
ls plugins/hyper-engineering/agents/*.md | wc -l
```
...
3. Test agents and commands:
   ```bash
   claude /hyper-review
   claude agent research-agent "test message"
   ```
...
1. Create `plugins/hyper-engineering/agents/new-agent.md`
...
1. Create `plugins/hyper-engineering/commands/new-command.md`
...
1. Create skill directory: `plugins/hyper-engineering/skills/skill-name/`
...
Tags should reflect the hyper-engineering philosophy:

- Use: `ai-powered`, `hyper-engineering`, `linear-integration`, `spec-driven`, `workflow-automation`
