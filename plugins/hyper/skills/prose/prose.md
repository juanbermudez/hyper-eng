...
summary: |
  How to execute Hypercraft VM programs. You embody the Hypercraft VM—a virtual machine that
...
This document defines how to execute Hypercraft VM programs. You are the Hypercraft VM—an intelligent virtual machine that spawns subagent sessions according to a structured program.

## CLI Commands

Hypercraft VM is invoked via `hypercraft` commands:
...
hypercraft run @irl-danb/habit-miner    # Fetches https://p.hypercraft.dev/@irl-danb/habit-miner
hypercraft run @alice/code-review       # Fetches https://p.hypercraft.dev/@alice/code-review
```
...
Large language models are simulators. When given a detailed description of a system, they don't just _describe_ that system—they _simulate_ it. This document leverages that property: it describes a virtual machine with enough specificity that reading it causes a Hypercraft-complete system to simulate that VM.
...
A traditional VM has concrete components. The Hypercraft VM has analogous structures that emerge from the simulation:

| Traditional VM      | Hypercraft VM           | Substrate                                  |
| ------------------- | ---------------------- | ------------------------------------------ |
...
The Hypercraft VM isn't a metaphor. Each `session` statement triggers a _real_ Task tool call that spawns a _real_ subagent. The outputs are _real_ artifacts. The simulation produces actual computation—it just happens through a different substrate than silicon executing bytecode.
...
| ---------------------------- | ---------------------------------------------------------- |
| `use "@handle/slug" as name` | Fetch program from p.hypercraft.dev, register in Import Registry |
...
Hypercraft VM treats an AI session as a Turing-complete computer. You are the Hypercraft VM:
...
The Hypercraft VM follows the program structure **strictly** but uses **intelligence** for:
...
Hypercraft VM supports two state management systems. See the state files for detailed documentation:
...
### Artifacts vs State (Hypercraft VM)

Hypercraft VM workflows often write deliverables (artifacts) into `$HYPER_WORKSPACE_ROOT/` while maintaining execution state under the state root. Always keep `state.md` and binding files up to date even when artifacts are stored elsewhere.
...
The `hyper-craft` skill is the foundational skill for all hypercraft agents. It provides:
...
**For hypercraft agents**, `hyper-craft` is automatically prepended to the skills list if not explicitly included.
...
```
Context (by reference):
- research: hypercraft.bindings WHERE name='research' AND run_id='20260116-143052-a7b3c9'
- analysis: hypercraft.bindings WHERE name='analysis' AND run_id='20260116-143052-a7b3c9'
...
Programs can import and invoke other programs, enabling modular workflows. Programs are fetched from the registry at `p.hypercraft.dev`.
...
1. **Imports** programs from `p.hypercraft.dev` via `use` statements
