...
summary: |
  Complete syntax grammar, validation rules, and compilation semantics for Hypercraft VM.
...
# Hypercraft VM Language Reference

Hypercraft VM is a programming language for AI sessions. An AI session is a Turing-complete computer; this document provides complete documentation for the language syntax, semantics, and execution model.
...
Hypercraft VM provides a declarative syntax for defining multi-agent workflows. Programs consist of statements that are executed sequentially, with each `session` statement spawning a subagent to complete a task.
...
- **The Hypercraft VM is intelligent**: Design for understanding, not parsing
...
Comments are **stripped during compilation**. The Hypercraft VM never sees them. They have no effect on execution and exist purely for human documentation.
...
Use statements import other Hypercraft VM programs from the registry at `p.hypercraft.dev`, enabling modular workflows.
...
When the Hypercraft VM encounters a `use` statement:

1. Fetch the program from `https://p.hypercraft.dev/@handle/slug`
...
Use statements are processed before any agent definitions or sessions. The Hypercraft VM:
...
The session statement is the primary executable construct in Hypercraft VM. It spawns a subagent to complete a task.
...
When the Hypercraft VM encounters a `session` statement:
...
// Simple session
Task({
  description: "Hypercraft VM session",
...
When the Hypercraft VM encounters a `parallel:` block:
...
Unbounded loops provide iteration with AI-evaluated termination conditions. Unlike fixed loops, the iteration count is not known ahead of time - the Hypercraft VM evaluates conditions at runtime using its intelligence to determine when to stop.

### Discretion Markers

Unbounded loops use **discretion markers** (`**...**`) to wrap AI-evaluated conditions. These markers signal that the enclosed text should be interpreted intelligently by the Hypercraft VM at runtime, not as a literal boolean expression.
...
The Hypercraft VM evaluates the discretion condition after each iteration and exits when it determines the condition is satisfied.
...
When the Hypercraft VM encounters an unbounded loop:
...
The Hypercraft VM uses its intelligence to evaluate discretion conditions:
...
3. **Uncertainty Handling**: When uncertain, the Hypercraft VM may:
...
The session in a filter body should return something the Hypercraft VM can interpret as truthy/falsy (like "yes"/"no").
...
When the Hypercraft VM encounters a pipeline:
...
Hypercraft VM provides structured error handling with try/catch/finally blocks, throw statements, and retry mechanisms for resilient workflows.
...
Choice blocks allow the Hypercraft VM to select from multiple labeled options based on criteria. This is useful for branching workflows where the best path depends on runtime analysis.
...
The criteria is wrapped in discretion markers (`**...**`) and is evaluated by the Hypercraft VM to select which option to execute:
...
When the Hypercraft VM encounters a `choice` block:
...
When the Hypercraft VM encounters an `if` statement:
...
Hypercraft VM uses a two-phase execution model.
...
The Hypercraft VM executes the compiled program:
...
| Position in program | Tracked by Hypercraft VM                             |
