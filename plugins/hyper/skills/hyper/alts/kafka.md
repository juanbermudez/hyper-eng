...
role: experimental
summary: |
  Kafka register for Hypercraft VM—a bureaucratic/absurdist alternative keyword set.
  Clerks, proceedings, petitions, and statutes. For benchmarking against the functional register.
status: draft
requires: prose.md
---

# Hypercraft VM Kafka Register
...
An alternative register for Hypercraft VM that draws from the works of Franz Kafka—The Trial, The Castle, "In the Penal Colony." Programs become proceedings. Agents become clerks. Everything is a process, and nobody quite knows the rules.
