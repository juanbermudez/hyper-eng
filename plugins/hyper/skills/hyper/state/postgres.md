...
status: experimental
summary: |
  PostgreSQL-based state management for Hypercraft VM programs. This approach persists
...
This document describes how the Hypercraft VM tracks execution state using a **PostgreSQL database**. This is an experimental alternative to file-based state (`filesystem.md`), SQLite state (`sqlite.md`), and in-context state (`in-context.md`).
...
**⚠️ Credentials are visible to subagents.** The `HYPERCRAFT_POSTGRES_URL` connection string is passed to spawned sessions so they can write their outputs. This means:

- Database credentials appear in subagent context and may be logged
- Treat these credentials as **non-sensitive**
- Use a **dedicated database** for Hypercraft VM, not your production systems
- Create a **limited-privilege user** with access only to the `hypercraft` schema

**Recommended setup:**
```sql
-- Create dedicated user with minimal privileges
CREATE USER hypercraft_agent WITH PASSWORD 'changeme';
CREATE SCHEMA hypercraft AUTHORIZATION hypercraft_agent;
GRANT ALL ON SCHEMA hypercraft TO hypercraft_agent;
-- User can only access the hypercraft schema, nothing else
```
...
createdb myproject
echo "HYPERCRAFT_POSTGRES_URL=postgresql://localhost/hypercraft" >> .prose/.env
```
...
echo "HYPERCRAFT_POSTGRES_URL=postgresql:///hypercraft" >> .prose/.env
```
...
```bash
# Example: Neon
echo "HYPERCRAFT_POSTGRES_URL=postgresql://user:pass@ep-name.us-east-2.aws.neon.tech/neondb?sslmode=require" >> .prose/.env
```
...
1. `HYPERCRAFT_POSTGRES_URL` in `.prose/.env`
2. `HYPERCRAFT_POSTGRES_URL` in shell environment
3. `DATABASE_URL` in shell environment (common fallback)

### Security: Add to .gitignore

```gitignore
# Hypercraft VM sensitive files
.prose/.env
.prose/runs/
```
...
|----------------|-------------|
| **Schema initialization** | Create `hypercraft` schema and tables at run start |
...
Location: hypercraft.bindings WHERE name='research' AND run_id='20260116-143052-a7b3c9' AND execution_id IS NULL
...
Location: hypercraft.bindings WHERE name='result' AND run_id='20260116-143052-a7b3c9' AND execution_id=43
Execution ID: 43
...
The VM initializes these tables using the `hypercraft` schema. This is a **minimum viable schema**—extend freely.

```sql
-- Create dedicated schema for Hypercraft VM state
CREATE SCHEMA IF NOT EXISTS hypercraft;

-- Run metadata
CREATE TABLE IF NOT EXISTS hypercraft.run (
    id TEXT PRIMARY KEY,
    program_path TEXT,
    program_source TEXT,
...
CREATE TABLE IF NOT EXISTS hypercraft.execution (
    id SERIAL PRIMARY KEY,
    run_id TEXT NOT NULL REFERENCES hypercraft.run(id) ON DELETE CASCADE,
    ...
    error_message TEXT,
    parent_id INTEGER REFERENCES hypercraft.execution(id) ON DELETE CASCADE,
    metadata JSONB DEFAULT '{}'::jsonb
);

-- All named values (input, output, let, const)
CREATE TABLE IF NOT EXISTS hypercraft.bindings (
    name TEXT NOT NULL,
    run_id TEXT NOT NULL REFERENCES hypercraft.run(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS hypercraft.agents (
    name TEXT NOT NULL,
...
CREATE TABLE IF NOT EXISTS hypercraft.agent_segments (
    id SERIAL PRIMARY KEY,
...
-- Import registry
CREATE TABLE IF NOT EXISTS hypercraft.imports (
    alias TEXT NOT NULL,
    run_id TEXT NOT NULL REFERENCES hypercraft.run(id) ON DELETE CASCADE,
...
CREATE INDEX IF NOT EXISTS idx_execution_run_id ON hypercraft.execution(run_id);
CREATE INDEX IF NOT EXISTS idx_execution_status ON hypercraft.execution(status);
CREATE INDEX IF NOT EXISTS idx_execution_parent_id ON hypercraft.execution(parent_id) WHERE parent_id IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_execution_metadata_gin ON hypercraft.execution USING GIN (metadata jsonb_path_ops);
CREATE INDEX IF NOT EXISTS idx_bindings_run_id ON hypercraft.bindings(run_id);
CREATE INDEX IF NOT EXISTS idx_bindings_execution_id ON hypercraft.bindings(execution_id) WHERE execution_id IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_agents_run_id ON hypercraft.agents(run_id) WHERE run_id IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_agents_project_scoped ON hypercraft.agents(name) WHERE run_id IS NULL;
CREATE INDEX IF NOT EXISTS idx_agent_segments_lookup ON hypercraft.agent_segments(agent_name, run_id);
```
...
WITH RECURSIVE scope_chain AS (
  ...
  SELECT id, parent_id FROM hypercraft.execution WHERE id = 43
  UNION ALL
  -- Walk up to parent
  SELECT e.id, e.parent_id
  FROM hypercraft.execution e
  JOIN scope_chain s ON e.id = s.parent_id
)
SELECT b.* FROM hypercraft.bindings b
WHERE b.name = 'result'
...
SELECT * FROM hypercraft.bindings
WHERE name = 'result'
...
```bash
# Initialize schema
psql "$HYPERCRAFT_POSTGRES_URL" -f schema.sql

# Register a new run
psql "$HYPERCRAFT_POSTGRES_URL" -c "
  INSERT INTO hypercraft.run (id, program_path, program_source, status)
  VALUES ('20260116-143052-a7b3c9', '/path/to/program.prose', 'program source...', 'running')
"

# Update execution position
psql "$HYPERCRAFT_POSTGRES_URL" -c "
  INSERT INTO hypercraft.execution (run_id, statement_index, statement_text, status, started_at)
  VALUES ('20260116-143052-a7b3c9', 3, 'session \"Research AI safety\"', 'executing', NOW())
"

# Read a binding
psql "$HYPERCRAFT_POSTGRES_URL" -t -A -c "
  SELECT value FROM hypercraft.bindings WHERE name = 'research' AND run_id = '20260116-143052-a7b3c9'
"

# Check parallel branch status
psql "$HYPERCRAFT_POSTGRES_URL" -c "
  SELECT metadata->>'branch' AS branch, status FROM hypercraft.execution
...
psql "$HYPERCRAFT_POSTGRES_URL" -c "
  INSERT INTO hypercraft.bindings (name, run_id, execution_id, kind, value, source_statement)
  VALUES (
    'research',
...
Read your current state:
  psql "$HYPERCRAFT_POSTGRES_URL" -t -A -c "SELECT memory FROM hypercraft.agents WHERE name = 'captain' AND run_id = '20260116-143052-a7b3c9'"

Update when done:
  psql "$HYPERCRAFT_POSTGRES_URL" -c "UPDATE hypercraft.agents SET memory = '...', updated_at = NOW() WHERE name = 'captain' AND run_id = '20260116-143052-a7b3c9'"

Record this segment:
  psql "$HYPERCRAFT_POSTGRES_URL" -c "INSERT INTO hypercraft.agent_segments (agent_name, run_id, segment_number, prompt, summary) VALUES ('captain', '20260116-143052-a7b3c9', 3, '...', '...')"
```
...
SELECT memory FROM hypercraft.agents WHERE name = 'advisor' AND run_id IS NULL;

-- Update project-scoped agent memory
UPDATE hypercraft.agents SET memory = '...' WHERE name = 'advisor' AND run_id IS NULL;
```
...
[Binding] research = <stored in hypercraft.bindings>
```
...
INSERT INTO hypercraft.execution (run_id, statement_index, statement_text, status, started_at, metadata)
...
INSERT INTO hypercraft.execution (run_id, statement_index, statement_text, status, started_at, parent_id, metadata)
VALUES
...
Each subagent writes to a different row in `hypercraft.bindings`. PostgreSQL's row-level locking means **no blocking**:
...
-- Mark run as failed
UPDATE hypercraft.run SET status = 'failed' WHERE id = '20260116-143052-a7b3c9';
```
...
DELETE FROM hypercraft.agents WHERE run_id = '20260116-143052-a7b3c9';

-- Delete a specific project-scoped agent (user-initiated)
DELETE FROM hypercraft.agents WHERE name = 'old_advisor' AND run_id IS NULL;
```
...
```sql
INSERT INTO hypercraft.bindings (name, run_id, kind, value, attachment_path, source_statement)
VALUES (
  'full_report',
  '20260116-143052-a7b3c9',
  'let',
...
FROM hypercraft.execution
WHERE run_id = '20260116-143052-a7b3c9' AND status = 'executing'
ORDER BY id DESC LIMIT 1;

-- Get all completed bindings
SELECT name, kind, value, attachment_path FROM hypercraft.bindings
WHERE run_id = '20260116-143052-a7b3c9';

-- Get agent memory states
SELECT name, scope, memory FROM hypercraft.agents
...
CREATE TABLE IF NOT EXISTS hypercraft.x_metrics (
    id SERIAL PRIMARY KEY,
    run_id TEXT REFERENCES hypercraft.run(id) ON DELETE CASCADE,
    execution_id INTEGER REFERENCES hypercraft.execution(id) ON DELETE CASCADE,
...
-- Add custom column
ALTER TABLE hypercraft.bindings ADD COLUMN IF NOT EXISTS token_count INTEGER;

-- Create index for common query
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_bindings_created ON hypercraft.bindings(created_at);
```
